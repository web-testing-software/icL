...
Веб-элементы могут быть описаны литералом \mintinline{icl}{метод@предел[селектор]}, где \mintinline{icl}{метод} — одно из следующих фиксируемых слов: \mintinline{icl}{css}, \mintinline{icl}{xpath}, \mintinline{icl}{link}, \mintinline{icl}{links}, \mintinline{icl}{tag}, \mintinline{icl}{tags}. \mintinline{icl}{@предел} — локальная переменная содержащая веб-элемент. \mintinline{icl}{селектор} — селектор, набор последовательных символов, по которому определяется какие элементы будут выбраны, для каждого метода нужен специфический селектор.
...
В контексте веб-элементов появляются новый оператор \mintinline{icl}{[element ...] :}\\*\mintinline{icl}{element}.
...
Возвращает строку, содержащая код base64 скриншота элемента. Его можно сохранить как изображение используя \mintinline{icl}{Make.image (base64 : string, path : string) :}\\*\mintinline{icl}{void}.
...
Возвращает новая коллекция, содержащая все элементы, которые содержат текст подходящая по шаблону \mintinline{icl}{template}.
