...
Codul, care generează erori se numește emitor. El se declară folosind următoarea sintaxă \mintinline{icl}{emiter {```code```}}. Semnalul generat este prins de colector. Colectorii pot fi cîțiva. Ei declară folosind următoarea sintaxă \mintinline{icl}{slot:exception {```code```}},
...
Pentru a genera semnale proprii folosiți comanda \mintinline{icl}{emit:exception (description : string)}, unde \mintinline{icl}{exception} e numele semnalului, dar \mintinline{icl}{description} e descrierea.

Pentru a bloca toate semnalele se folosește jammer, el are următoarea sintaxă \mintinline{icl}{jammer {```code```}}. Semnalele erorilor semantice și de sintaxă, și \mintinline{icl}{emit:Exit} de asemenea, nu pot fi oprite de jammer.
