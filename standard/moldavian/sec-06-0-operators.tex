...
Returnează \true{} dacă \code{obj1} și \code{obj2} имеют одни și те же поля, значения полях из первого объекта совпадает с значениям одноимённых полях второго объекта, în caz contrar \false{}.
...
Returnează \true{} dacă \code{set1} și \code{set2} имеют один și тоже заголовок, всё объекты из первой множестве приличествуют во вторым și наоборот, în caz contrar \false{}.
...
Returnează \true{} dacă количество объектов во втором объекте отличается от количества объектов в первом или существует объект присутствующий только в одной множестве, în caz contrar \false{}.

\subsubsection{\lstinline`<set>s << <object>obj : bool`}

Returnează \true{}, dacă множество \code{s} содержит объект \code{obj}, în caz contrar \false{}.

\subsubsection{\lstinline|<set>set1 << <set>set2 : bool|}

Returnează \true, dacă множество \code{set1} содержит подмножество \code{set2}, în caz contrar \false.

\subsubsection{\lstinline`<set>s <* <object>subobj : bool`}

Returnează \true{}, dacă множество \code{s} содержит под объект \code{subobj}, în caz contrar \false{}.

\subsubsection{\lstinline`<object>obj <* <object>subobj : bool`}

Returnează \true{} dacă каждое поле объекта \code{subobj} содержится в объекте \code{obj} și значение одноимённых полях совпадает, în caz contrar \false{}.

\subsubsection{\lstinline`<set>set1 + <set>set2 : set`}

Returnează новое множество содержащая все объекты множеств \code{set1} și \code{set2}.

\subsubsection{\lstinline`<set>set1 - <set>set2 : set`}

Returnează новое множество содержащая все объекты множества \code{set1}, отсутствующих в множестве \code{set2}, și все объекты множестве \code{set2}, отсутствующих в \code{set1}.

\subsubsection{\lstinline`<set>set1 \\ <set>set2 : set`}

Returnează новое множество содержащая все объекты множества \code{set1}, отсутствующих в множестве \code{set2}.

\subsubsection{\lstinline`<set>set1 * <set>set2 : set`}

Returnează новое множество содержащая все объекты , которые приличествуют в \code{set1} și \code{set2}.

\subsubsection{\lstinline`<set>set1 ** <set>set2 : bool`}

Returnează \true, dacă множества \code{set1} și \code{set2} пересекаются, în caz contrar \false.
