...
\section{Условные операторы}
	...
	Условные структуры имеют условие, которое будет вычислено. А также и блоки команд, чья выполнение зависит от значении условии.
...
\subsubsection{\lstinline`if else`}
	...
	Конструкция \code{if else} выглядит следующим образом:
\begin{lstlisting}[numbers=none]
if (condition) {
	commands1
}
...
\begin{icEnum}
	...
	\item - Циклы for, while, do while, filter, range
