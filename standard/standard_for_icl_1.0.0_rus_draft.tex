...
\lstdefinelanguage{icL}{
	...
	keywords=[2]{bool, int, double, string, list, element, set, item, object, function, void, request, char},
...
\begin{icItems}
	\item \code{'typeName : string} - при чтении получаем строку содержащая названия типа хранимых данных;
	\item \code{'typeId : int} - при чтении получаем число содержащее идентификатор типа хранимых данных;
	\item \code{'rValue : bool} - при чтении получаем \code{true} если значение предназначено для правой части операции присваивания, иначе \code{false};
	\item \code{'readOnly : bool} - при чтении получаем \code{true} если объект доступен только для чтения, иначе \code{false};
	\item \code{'lValue : bool} - при чтении получаем \code{true} если объект доступен только для чтения и записи, иначе \code{false};
	\item \code{'link : bool} - при чтении получаем \code{true} если значение объекта хранится во внешнем контейнере и изменение значении объекта будет изменить данные во внешней среде, иначе \code{false}.
\end{icItems}
...
Методы позволяют изменить состояние объекта. Все типы данных имеют только один общий метод \code{.ensureRValue}, который гарантирует что изменение значению объекту не будет изменить данные во вне текущего контекста.
...
В icL данные по умолчанию не преобразуется. На пример сложить \code{int} с \code{double} нельзя, такой оператор не определён, в этом случае можно преобразовать \code{int} в \code{double} или \code{double} в \code{int}, в зависимости от типа нужного результата.
...
{\color{red} Важно:} не все типы данных можно запросить, а только \code{bool}, \code{int}, \code{double}, \code{string}, \code{list} и \code{object}.
...
Каждое JS-Значение имеет геттер, но только переменные доступны для записи и чтения имеют сеттер. Геттеры и сеттеры являются фрагментами кода на языке JavaScript. В сеттере установленное значение передаётся следующим кодом \code{@{value}} Описать JS-Значение можно следующим образом -
\begin{lstlisting}[numbers=none]
$value {getter, setter};
\end{lstlisting}
...
\code{[w3c]} Команда \lstinline|$run| получает переменное количество переменных. Они передаются в коде на языке JavaScript в виде аргументов вызова функций. Доступ к ним осуществляется с помощью переменной \code{arguments}.
...
\begin{icItems}
	\item \code{name} - {\bf название функции};
	\item \code{parameters} - {\bf список параметров}. Когда функция вызывается вы передаёте значения параметру. Список параметров определяет тип, количество и порядок параметров. Список параметров необязателен;
	\item \code{return_type} - {\bf тип данных которых} функция будет возвращать. Если функция ничего не возвращает тип данных можно пропустить или указать явно \code{void}. Чтобы возвращать значение используется команда \code{@ = value};
	\item \code{commands} - {\bf тело функции} содержит команды, которые определяют что делает функция.
\end{icItems}
...
!name arguments;
\end{lstlisting}
где \code{name} - {\bf название функции}, \code{arguments} - {\bf список аргументов} (он должен быть совместим со списком параметров функции).
...
{\bf Строки} (тип \code{string}) позволяет хранить и манипулировать фрагменты текста. 

В контексте строк появляется ещё один тип данных \code{char}, он представляет символ в составе строки, и позволяет редактировать строку.

Тип данных \code{char} имеет свойство \code{code : int} - код символа и его можно преобразовать в \code{string}.

Строки имеют следующее дополнительные свойства:
\begin{icItems}
	\item \code{'empty : bool} - "пустая", строка считает пустой если количество содержащих символов равна нулю (доступно только для чтения);
	\item \code{'length : int} - "длина", длина строки равна количеству символов в строке;
	\item \code{'last : char} - последний символ строки.
	\item \lstinline|'(<int> n) : char| - n-й символ строки. n должен быть целым литералов, примеры \lstinline|@str'0; @string'2|.
\end{icItems}

Строки имеют следующий набор дополнительных методов:
\begin{icItems}
	\item \lstinline|.append <string>str : string| - вставит стоку в конце строки;
	\item \lstinline|.append <char>ch : string| - вставит символ в конце строки;
	\item \lstinline|.at <int>i : char| - возвращает ссылка на \code{i}-й символ;
	\item \lstinline|.beginsWith <string>str : bool| - возвращает \code{true} если начало строки совпадает с \code{str}, иначе \code{false};
	\item \lstinline|.beginsWith <char>ch : bool|  - возвращает \code{true} если первый символ строки равен  \code{ch}, иначе \code{false};
	\item \lstinline|.compare <string>str <bool>caseSensitive = true : bool| - сравнивает строки, возвращает \code{true} если они равны, иначе \code{false}; аргумент \code{caseSensitive} может быть пропущен, если установить его в \code{false} то регистр букв будет проигнорирован;
	\item \lstinline|.count <string>str : int| - считает сколько раз подстрока \code{str} встречается в строке;
	\item \lstinline|.count <char>ch : int| - считает сколько раз символ \code{ch} встречается в строке;
	\item \lstinline|.endsWith <string>str : bool| - возвращает \code{true} если конец строки совпадает с \code{str}, иначе \code{false};
	\item \lstinline|.endsWith <char>ch : bool| - возвращает \code{true} если последний символ строки равен \code{ch}, иначе \code{false};
	\item \lstinline|.indexOf <string>str <int>startPos = 0 : int| - возвращает индекс первой нахождения подстроки \code{str} в строке, ища вперёд с позиции \code{startPos}, если подстрока не найдено возвращает -1;
	\item \lstinline|.indexOf <char>ch <int>startPos = -1 : int| - возвращает индекс первой нахождения символа \code{ch} в строке, ища вперёд с позиции \code{startPos}, если символ не найден возвращает -1;
	\item \lstinline|.insert <string>str <int>pos : string| - вставит строку \code{str} в позиции \code{pos};
	\item \lstinline|.insert <char>ch <int>pos : string| - вставит символ \code{ch} в позиции \code{pos};
	\item \lstinline|.lastIndexOf <string>str <int>startPos = -1 : int| - возвращает индекс первой нахождения подстроки \code{str} в строке, ища назад с позиции \code{startPos}, если подстрока не найдена возвращает -1;
	\item \lstinline|.lastIndexOf <char>ch <int>startPos = -1 : int| - возвращает индекс первой нахождения символа \code{ch} в строке, ища назад с позиции \code{startPos}, если символ не найден возвращает -1;
	\item \lstinline|.left <int>n : string| - возвращает подстроку содержащая первый \code{n} символы;
	\item \lstinline|.leftJustified <int>width <string>fillChar <bool>truncate = false : string| - возвращает строку длины \code{width}, содержащую эту строку, в конце ставится \code{width - .length} символов \code{fillChar}; если \code{truncate == true} и \code{width < .length} то последние \code{.length - width} символы будут удалены;
	\item \lstinline|.mid <int>pos <int>n = -1 : string| - возвращает строку содержащую \code{n} символы строки, начиная с позиции \code{pos}; будет возвращена пустая строка если установленный интервал выходит за рамки строки; Если \code{n == -1} то будут возвращены все доступные символы начиная с позицией \code{pos};
	\item \lstinline|.prepend <string>str : string| - вставит строку \code{str} в начале строки;
	\item \lstinline|.prepend <char>ch : string| - вставит символ \code{ch} в начале строки;
	\item \lstinline|.remove <int>pos <int>n : string| - удаляет \code{n} символы строки начиная с позицией \code{pos};
	\item \lstinline|.remove <string>str <bool>caseSensitive = true : string| - удаляет каждое нахождения подстроки \code{str} в строке; аргумент \code{caseSensitive} может быть пропущен, если установить его в \code{false} то регистр букв будет проигнорирован;
	\item \lstinline|.replace <int>pos <int>n <string>after : string| - заменяет интервал установленный позицией \code{pos} и количеством символов \code{n} строкой \code{after};
	\item \lstinline|.replace <string>before <string>after : string| - заменяет каждое нахождение подстроки \code{before} подстрокой \code{after};
	\item \lstinline|.replace <char>before <char>after : string| - заменяет каждый символ \code{before} на символе \code{after};
	\item \lstinline|.right <int>n : string| - возвращает строку содержащую последние \code{n} символы строки;
	\item \lstinline|.rightJustified <int>width <string>fillChar <bool>truncate = false : string| - возвращает строку длины \code{width}, содержащую эту строку, в начале ставится \code{width - .length} символов \code{fillChar}; если \code{truncate == true} и \code{width < .length} то первые \code{.length - width} символы будут удалены;
	\item \lstinline|.split <string>separator <bool>keepEmptyParts = true <bool>caseSensitive = true : list| - разрывает строку на подстроки при каждой нахождения подстроки \code{separator} и собирает список из этих строк; если \code{separator} не встречается ни разу, будет возвращён список из одной строки - этой строки;
	\item \lstinline|.split <char>separator <bool>keepEmptyParts = true <bool>caseSensitive = true : list| - разрывает строку на подстроки при каждой нахождения символа \code{separator} и собирает список из этих строк; если \code{separator} не встречается ни разу, будет возвращён список из одной строки - этой строки;
	\item \lstinline|.substring <int>begin <int>end : string| - возвращает подстроку содержащую символы строки, от позиции \code{begin} до \code{end}.
\end{icItems}
