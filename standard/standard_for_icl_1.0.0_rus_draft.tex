...
\item
	\lstinline|string.prepend <char>ch : string|;
\item
...
\item
	\lstinline|string.replace <char>before <char>after : string|;
\item
...
\item
	\lstinline|string.split <char>separator <bool>keepEmptyParts = true <bool>caseSensitive = true : list|;
\item
	\lstinline|string.substring <int>begin <int>end : string|.
\end{icItems}

Некоторые методы пропущены, они будут представлены в главе \ref{regex}; Код, использующий выше перечисленных методов представлен на листинге \ref{stringprop}.

\begin{lstlisting}[caption=Методы класса string, label=stringprop]
@empty = "";
@fonts = "Arial, Helvetica, Times, Courier";
@index = 2;
...
\subsubsection{\lstinline|string.append <char>ch : string|}

Вставит символ в конце строки.
...
\subsubsection{\lstinline|string.beginsWith <char>ch : bool|}

Возвращает \true{}, если первый символ строки равен  \code{ch}, иначе \false{}.
...
\subsubsection{\lstinline|string.count <char>ch : int|}

Считает сколько раз символ \code{ch} встречается в строке.

\subsubsection{\lstinline|string.endsWith <string>str : bool|}

Возвращает \true{}, если конец строки совпадает с \code{str}, иначе \false{}.

\subsubsection{\lstinline|string.endsWith <char>ch : bool|}

Возвращает \true{}, если последний символ строки равен \code{ch}, иначе \false{}.
...
\subsubsection{\lstinline|string.indexOf <char>ch <int>startPos = -1 : int|}

Возвращает индекс первой нахождения символа \code{ch} в строке, ища вперёд с позиции \code{startPos}, если символ не найден возвращает -1.

\subsubsection{\lstinline|string.insert <string>str <int>pos : string|}

Вставит строку \code{str} в позиции \code{pos}.

\subsubsection{\lstinline|string.insert <char>ch <int>pos : string|}

Вставит символ \code{ch} в позиции \code{pos}.
...
\subsubsection{\lstinline|string.lastIndexOf <char>ch <int>startPos = -1 : int|}

Возвращает индекс первой нахождения символа \code{ch} в строке, ища назад с позиции \code{startPos}, если символ не найден возвращает -1.
...
\subsubsection{\lstinline|string.prepend <char>ch : string|}

Вставит символ \code{ch} в начале строки.
...
\subsubsection{\lstinline|string.replace <char>before <char>after : string|}

Заменяет каждый символ \code{before} на символе \code{after}.
...
Внутренний символ может быть не только \code{/}, но и любой другой на свой вкус, кроме апострофа (\lstinline|'|) он конфликтует с оператором {\it корень} (\lstinline|/'|). Например  \lstinline|/"pattern"/mods|, \lstinline|/:pattern:/mods|, \lstinline|/$pattern$/mods|.
