...
void If::parseLogicExp () {
	if (isLogicExp) {
		...
		auto *single = new logic::rich::SingleBlock{ logic::RichBlock::OperationType::NotNot };
...
void If::processContains (Operator &op, const QChar &next, int i) {
	if ( ( next == '<' || next == '=' ) && op.rank < 2) {
		op.type		= next == '<' ? OperatorType::Contains : OperatorType::ContainsFragment;
		op.rank		= 2;
		op.position = i;
	}
}
...
logic::LogicBlock * If::returnRank1 (Operator &op, memory::CodeFragment &fn) {
	...
	if (!isOk) {
		emit exception ({ -201,
						  QString ("Rank 1 operators (! and !!) givis one operand, placed on right. "
