...
bool Assign::execute () {
	if (m_next->role () != Role::Object) {
		emit exception ({ -204, "The right operand of assign operation must be an `Object`" });
		return false;
	}
	...
	if (left->isReadOnly () ) {
		emit exception ({ -205, "The left operand of assign operation must be muttable" });
		return false;
	}
	...
	if (right->type () == memory::Type::Void) {
		if (left->type () == memory::Type::Void) {
			if (right->isLValue () ) {
				emit exception ({ -404,
								  QString ("Variable %1 not found")
								  .arg (right->getVarName () ) });
			}
			else {
				emit exception ({ -400, "Variables of type Void cannot be created" });
			}
		}
		else {
			emit exception ({ -206,
							  QString ("Void cannot be assigned to variable of type %1")
