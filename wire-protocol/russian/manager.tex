...
\begin{icItems}
	\item \jsinline{result} (строка) - результат: \jsinline{"successful"} - пройден удачно, \jsinline{"test-failed"} - тест упал из-за ошибки в коде, \jsinline{"assert-failed"} - тест упал при проверке утверждений, \jsinline{"unknown"} - неизвестный результат;
	\item \jsinline{session} (целое число) - идентификатор сессии;
